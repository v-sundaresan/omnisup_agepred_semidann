...
def train_semidann_omnisup(args, names, dir_checkpoint, models, batch_size, num_epochs, device,
    ...
    [train_names_source, train_names_target, val_names_source, val_names_target] = names
    ...
    train_names_lab_target = train_names_target[:int((len(train_names_target)*args.target_label_prop)//1)]
    val_names_lab_target = val_names_target[:int((len(val_names_target) * args.target_label_prop) // 1)]
    train_names_unlab_target = train_names_target[int((len(train_names_target) * args.target_label_prop) // 1) :]
    val_names_unlab_target = val_names_target[int((len(val_names_target) * args.target_label_prop) // 1) :]
    ...
    if save_resume:
        try:
            if dir_checkpoint is not None:
                ckpt_path = os.path.join(dir_checkpoint, 'tmp_model_omni_semiDANN_withdom.pth')
            else:
                ckpt_path = os.path.join(os.getcwd(), 'tmp_model_omni_semiDANN_withdom.pth')
    ...
    for epoch in range(start_epoch, num_epochs+1):
        ...
        for i in range(num_iters):
            ...
            sourcedata, train_src_gens, train_tgt_gens, val_gens = \
                get_training_validation_batch_generators(args, trainnames, valnames, epoch-1, i, mode)
            ...
            for j in range(numsteps):
                ...
                if epoch <= pretrain_epochs: # For pre-training
					...
					# Omni-supervised learning label generation using old truenet model
        ...
        if (epoch % 1 == 0) and (epoch > args.pretrain_epochs//3):
            if save_checkpoint:
                try:
        ...
        if save_checkpoint:
            np.savez(os.path.join(dir_checkpoint, 'losses_omni_semiDANN.npz'), train_seg_loss=lossvals,
                     train_dom_loss=losses_dom_val, val_seg_loss=val_lossvals, val_dom_loss=val_losses_dom_val)
            np.savez(os.path.join(dir_checkpoint, 'validation_perf_omni_semiDANN.npz'), val_seg_score=val_dice,
